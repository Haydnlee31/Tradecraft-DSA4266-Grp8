...
from src.models.dataset import SPLITS_DIR, build_dataloaders
...
def main() -> None:
    ...
    parser.add_argument(
        "--checkpoint-dir",
        type=Path,
        default=ROOT / "models" / "checkpoints",
    )
    ...
    history = trainer.fit(
        data["loaders"]["train"], data["loaders"]["val"], epochs=args.epochs
    )
    ...
    out_path = args.reports_dir / f"{config.name}_{args.loss}_seed{args.seed}.json"
    json_args = {
    ...
    result = {
        ...
        "data": {
            ...
            "scaler_path": str(data["scaler_path"].resolve()),
            "feature_columns": data["feature_columns"],
            "classes": CLASSES,
        },

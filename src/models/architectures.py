...
HEAVY_CONFIG = MLPConfig(
    name="centralized_heavy", hidden_dims=(512, 256, 128, 64), dropout=0.3
)
...
class MLPClassifier(nn.Module):
    ...
    def parameter_bytes(self) -> int:
        """Exact size of parameter tensors (not runtime memory) in bytes."""
        return sum(p.numel() * p.element_size() for p in self.parameters())


def config_for_variant(variant: str) -> MLPConfig:
    """Resolve a CLI/report lane name to its single canonical architecture."""
    if variant == "heavy":
        return HEAVY_CONFIG
    if variant == "light":
        return LIGHT_CONFIG
